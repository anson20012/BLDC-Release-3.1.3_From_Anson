...
static const uint8_t pwm_io_state[7] = { 0, 0x02, 0x04, 0x04, 0x01, 0x01, 0x02 };  // H-bridge low-side driver(PA0¡¢PA1¡¢PA2) output high in GPIO mode
										  ...
										  //          U        U        V        V
static const uint16_t psel_mask_port_a[7] = { 0, 0x1001,  0x1001,  0x4004,  0x4004, 0x0010, 0x0010};  // H-bridge driver use PWM with death-time mode
//static const uint16_t psel_mask_port_a[7] = { 0, 0x1000, 0x1000, 0x4000, 0x4000 , 0x0000, 0x0000,};   // H-bridge driver just high-side(PA6¡¢PA7) use PWM mode
										//          							       W      W
static const uint16_t psel_mask_port_d[7] = { 0, 0x0005, 0x0005, 0x0005, 0x0005 , 0x4005, 0x4005};    // H-bridge driver just high-side(PD7) select to PAMA6,PD0\PD1 select to SWD mode
...
PAC5XXX_RAMFUNC void commutate(int hs)
{
	...
	if (hs <= 6)
	{
		...
		PAC5XXX_GPIOD->PSEL.s = psel_mask_port_d[hs];       // Set peripheral select state
...
void app_amplifier_gain_init(uint32_t app_amplifier_gain)
{
	...
	pac5xxx_tile_register_write(ADDR_CFGAIO4, (0x43|(motor_ptr->app_amplifier_gain_register<<3)));	//0x4A-->X1 //0x52-->X2 //0x5A-->X4 //0x62-->X8
	pac5xxx_tile_register_write(ADDR_CFGAIO5, 0x30);							//LPROT54
	pac5xxx_tile_register_write(ADDR_PROTINTM, 0x04);							//LPROT54-->0x04
#endif
}
...
void oc_reset(void)
{
	...
	while (pac5xxx_tile_register_read(ADDR_PROTSTAT));			// ADDR_PROTSTAT
...
void pwm_enable(void)
{
	...
	PAC5XXX_GPIOA->OUT.b &= ~0xC0;
	PAC5XXX_GPIOA->OUT.b &= ~0x07;
	PAC5XXX_GPIOD->OUT.b &= ~0x80;

	// Select GPIO peripheral for Port A
	PAC5XXX_GPIOA->PSEL.w |= 0x5015;
	PAC5XXX_GPIOD->PSEL.w |= 0x4000;    //PD7 --> PWMA6
